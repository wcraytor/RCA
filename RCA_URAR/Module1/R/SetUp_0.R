 ...
 flog.info(xx)
  ...
  projEnv$InteractionAggregationDF <-
    read.xlsx(projEnv$MlsDataFile,
              ...
              detectDates = TRUE)
  flog.info("Load SGMapping",name="Log2File")
...
CompressColumnNames <- function(cnms) {
  ...
  # Remove spaces, period, parentheses and pound signs from column names
  for (i in 1:nc) {
	flog.info(paste("ncx: ", i),name="Log2File")
    cnmsI <- cnms[i]
...
ConvertDatesToChar <- function(  df) {
  ...
  for (i in 1:nc) {
    if (t[i] == "Date") {
      flog.info(paste("i/t[i]: ", i, t[i]) )
      asChar <-    as.character(projEnv$MlsDataDF[, i], format = "%m/%d/%Y")
      df[, i] <- asChar
      flog.info(df[, i] )
    }
  }

    flog.info("",name="Log2File")
}

#' We need to convert Data Frame Dates to Char/string for the upload to SQLite to work correctly
#'  @return  Project Environment
ConvertDatesToCharAllDF <- function( ) {
  flog.info("ConvertDatesToChar MlsDataDF" )
  ConvertDatesToChar(  projEnv$MlsDataDF)

  flog.info("ConvertDatesToChar ProjectDF" )
  ConvertDatesToChar(  projEnv$ProjectDF)

  flog.info("ConvertDatesToChar InteractionAggregationDF" )
  ConvertDatesToChar(  projEnv$InteractionAggregationDF)

  flog.info("ConvertDatesToChar SG MlsMappingDF" )

  flog.info("ConvertDatesToChar AllowedInteractionsDF" )
...
WriteConfigMappingsDfToSqlite <- function()
{
  ...
  dbWriteTable(kveDB,
               ...
               projEnv$InteractionAggregationDF,
               overwrite = TRUE)
  #dbWriteTable(kveDB, "SGMapping", projEnv$SGMlsMappingDF, overwrite =
  #               TRUE)
  dbWriteTable(kveDB,
               "AllowedInteractions",
               projEnv$AllowedInteractionsDF,
		   ...
		   subjectCQA2) 
{
   ...
   # Move "AAA Rergression Feature" and "AggregateTo" columns of RegressionFields sheet to a separate data frame "projEnv$OneWayAggregationDF"
    RegressionFeature <- projEnv$RegressionFieldsDF[,"RegressionFeature"]
	 print("S2")
    AggregateTo <- projEnv$RegressionFieldsDF[,"AggregateTo"]
   print("S3")
  projEnv$OneWayAggregationDF <- data.frame(RegressionFeature,AggregateTo)
