...
Stage_2 <- function(  ) {
  ...
  for(u in 1:nu) {
    nm <- uv[u]
    if(nm != "." && nm != " ") {
      var <-  paste("projEnv$MlsDataDF$SG_",nm,sep="")
      if(u>1) {
        sgSum <- paste(sgSum,"+",sep="")
      }
      #print(var)
      sgSum <- paste(sgSum,var,"[r]",sep="")

      cVar <- paste("add_column(projEnv$MlsDataDF,","SG_",nm,"=1:nrows)",sep="")
      projEnv$MlsDataDF <- eval(parse(text=cVar))
      valueInitCmd <- paste("projEnv$MlsDataDF$SG_",nm," <- rep(0,nrows)",sep="")
      eval(parse(text=valueInitCmd))
    }
  }
  ...
  projEnv$AdjGrid <-  data.frame(Comp=1:maxRows,Address="",AdjName="",AdjValue=0,SGName="",SGValue=0,AdjRow=0)
  adjRow <- 1
  
  projEnv$MlsDataDF$SG_CQA = rep(0,nrows)
  ...
  for(r in 2:nrows) {
    ...
    for(n in 1:nlmv){
      ...
      SGNameFormula <- paste("projEnv$MlsDataDF$SG_",uName,"[",r,"]",sep="")
      print(paste("SGNameForumula: ", SGNameFormula,sep=""))
      SGNameFormula1 <- paste(SGNameFormula," <- ",SGNameFormula," + ",adjNameFormula,sep="")
      print(paste("SG Formula: ",SGNameFormula1))
      eval(parse(text=SGNameFormula1))
      if(r < 18) {
        print("r < 18 ....")
        adjRow <- adjRow + 1
        ...
        projEnv$AdjGrid$SGName[adjRow] <- uName
        projEnv$AdjGrid$SGValue[adjRow] <- eval(parse(text=SGNameFormula))
      }
    }
	...
	if(projEnv$ResidualMethod  == "Residual") {
          SGNameFormula1 <- paste("projEnv$MlsDataDF$SG_CQA","[",r,"]"," <- projEnv$MlsDataDF$SG_CQA","[",r,"]"," + projEnv$MlsDataDF$Adjstmts_CQA","[",r,"]",sep="")
	  } else {
	   SGNameFormula1 <- paste("projEnv$MlsDataDF$SG_CQA","[",r,"]"," <- projEnv$MlsDataDF$SG_CQA","[",r,"]"," + projEnv$MlsDataDF$Adjstmts_CQA2","[",r,"]",sep="")
    }
    ...
    if(r<18) {
      ...
      print(paste("SGNameFormula1",SGNameFormula1))
      projEnv$AdjGrid$Comp[adjRow]  <- r -1
      ...
      print("mm")
      projEnv$AdjGrid$SGName[adjRow]  <- "SG_CQA"
      print("mn")
    }
    ...
    if(r < 18) {
        print("xx")
        projEnv$AdjGrid$SGValue[adjRow]  <- eval(parse(text="projEnv$MlsDataDF$SG_CQA[r]"))
        print("xy")
    }
  }

  projEnv$AdjGrid  <- projEnv$AdjGrid [with(projEnv$AdjGrid ,order(Comp,SGName,AdjRow)),]
  flog.info( "Add and Initialize Net, Gros Total",name="Log2File")
  projEnv$MlsDataDF <- add_column(projEnv$MlsDataDF,SG_Net=1:nrows)
  projEnv$MlsDataDF <- add_column(projEnv$MlsDataDF,SG_Gross=1:nrows)
  projEnv$MlsDataDF <- add_column(projEnv$MlsDataDF,SG_SalePrice=1:nrows)
  projEnv$MlsDataDF$SG_Net <- rep(0,nrows)
  projEnv$MlsDataDF$SG_Gross <- rep(0,nrows)

  nu <- length(uv)
  flog.info(paste("uv Len: ",length(uv)),name="Log2File")
  projEnv$MlsDataDF$SG_SalePrice <- rep(0,nrows)
  ...
  for(r in 2:nrows) {
    ...
    for(n in 1:nu){
      ...
      SGNameFormula <- paste("projEnv$MlsDataDF$SG_",aName,"[",r,"]",sep="")

      if(n>1) {
        SGNetFormula <- paste(SGNetFormula,"+",sep="")
        SGGrossFormula <- paste(SGGrossFormula,"+",sep="")
      }
      SGNetFormula <- paste(SGNetFormula,SGNameFormula)
      SGGrossFormula <- paste(SGGrossFormula,"abs(",SGNameFormula,")")
    }
	
    SGNetFormula <-  paste(SGNetFormula,"+ projEnv$MlsDataDF$SG_CQA[r]",sep="")
    SGNetFormula <- paste("projEnv$MlsDataDF$SG_Net[",r,"]  <- ", SGNetFormula,sep="")
    SGGrossFormula <- paste("projEnv$MlsDataDF$SG_Gross[",r,"]  <- ", SGGrossFormula,sep="")
    flog.info(paste("Net Formula",SGNetFormula),name="Log2File")
    flog.info(paste("Gross Formula",SGGrossFormula),name="Log2File")
    eval(parse(text=SGNetFormula))
    eval(parse(text=SGGrossFormula))
    flog.info(paste("SalePrice:", projEnv$MlsDataDF$SalePrice[r]), name="Log2File")
    flog.info(paste("SG_Net: ",projEnv$MlsDataDF$SG_Net[r]),name="Log2File")
    
    projEnv$MlsDataDF$SG_SalePrice[r] <- projEnv$MlsDataDF$SalePrice[r] + projEnv$MlsDataDF$SG_Net[r]
