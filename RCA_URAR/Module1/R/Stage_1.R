...
initialize_InteractionTable <- function( ) {
  ...
  # A couple of vectors to store column and row names with RegressionFeature added
  regFldsC <- append(projEnv$RegrsInputFldNames, "RegressionFeature")
  regFldsR <- append(projEnv$RegrsInputFldNames, "RegressionFeature")
...
findContribVars <- function(  listModel) {
  ...
  for (j in 1:listModelLen) {
    ...
    for (n in 1:nr)  {
      ...
      searchFld <-  paste("[^A-z]", projEnv$RegressionFieldsDF[n, 1],"[^A-z]", sep = "")
...
callEarth <- function( ) {
  ...
  if (testStr != ".") {
      ...
      flog.info(paste("T3: ", makeFactor, sep = "",name="Log2File"))
  ...
  # Create a weights vector to allow us to include the subject as first row in the input matrix, without giving it
  # it weight in developing the regression model.  Yet, with the subject in the first row, we can apply the Model
  # to it to get an estimate of sale price and a residual
  # Not it would be better to add code to remove the first rows and store it separately  in a spreadsheet - but it is less
  # convenient to do that.
  weightsVector <- rep(10, nrows)
  ...
  try( modlm <- lm(SalePrice ~ . , data = X) )
  if(class(modlm) == "lm") { 
    pdf(paste(projEnv$MlsVersionFolder, '/LinearModels6.pdf', sep = "")) # make a PDF file to store plots
    
    par(bg= "lightgrey",col.lab="darkblue", col.axis="darkblue",col.main="darkblue",
        col.sub="darkblue",col="darkblue" ,mfrow=c(2, 2), mar=c(3, 3, 3, 1), mgp=c(1.5, 0.5, 0))
  ...
  projEnv$Mars <-
    list(
      ...
      modelList = listModel,
	  varmod = projEnv$MarsModel$varmod,
...
Stage_1 <- function() {
  ...
  Sys.sleep(3)
   ...
   projEnv$SaveSubjectResidual  <- 	  projEnv$MlsDataDF$Residual[1]
   projEnv$SaveSubjectResidualSF  <- 	  projEnv$MlsDataDF$ResidualSF[1]
  ...
  options(scipen = 999) 
  pdf(paste(projEnv$MlsVersionFolder, '/Varmod.pdf', sep = "")) # make a PDF file to store plots
   
     par(bg="#002060",col.lab="white", col.axis="white",col.main="white",   col.sub="white",col="white")
   	plot(projEnv$MarsModel$varmod)
    dev.off()
 ...
 projEnv$MlsDataDF <-
  ...
  ######################## CQA2 STUFF #####################
  # Order by ResidualSF
  # We don't want to reorder the first (subject) row, so make ResidualSF[1] large negative number to ensure it remains in first place.
  projEnv$MlsDataDF$ResidualSF[1] <- -99999999
  
  projEnv$MlsDataDF <-
      projEnv$MlsDataDF[order(projEnv$MlsDataDF$ResidualSF), ]
	
	projEnv$MlsDataDF$ResidualSF[1] <- projEnv$SaveSubjectResidualSF
	 nr <- nrow(projEnv$MlsDataDF)
    flog.info("Calculate CQA2", name = "Log2File")
    # Now calculate CQA2 starting with the 2nd row
    for (i in 2:nr) {
      projEnv$MlsDataDF$CQA2[i] = floor((((i - 1) / ((  nr - 1  ) / 1000)) / 1)  ) * 0.01
    }
	...
	# Create CqaMap for CQA2/ResidualSF
    projEnv$MlsDataDF$ResidualSF[1] <-
      projEnv$CqaToResidualSF$Find(projEnv$SubjectCQA2)
 
   # Create Residual2 values:
   for(p in 1:nr) {
   projEnv$MlsDataDF$Residual2[p] <-
     projEnv$MlsDataDF$ResidualSF[p] * projEnv$MlsDataDF$GLA[p]
   }
