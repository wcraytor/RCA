...
projEnv$BaseValue <- "1395961.95"
projEnv$RootPath <- "C:/Order_1/B/A/PVN2023/"
projEnv$Project  <- "KVE23_04_Pacifica_XXXXXXXXXXXXXXXXXX/"
...
projEnv$SGPrepDataFileName <- "SGPrepData.xlsx"
projEnv$AlamodeInputTemplateFileName <- "AlamodeInputTemplate.xlsx"
projEnv$AlamodeInputFileName <- "AlamodeInput.xlsx"
projEnv$RcaInputTemplateFileName <- "RcaInputTemplate.xlsx"
projEnv$RcaInputFileName <- "RcaInputA.xlsx"

projEnv$SGPrepDataFile <- paste(projEnv$MlsFolder,projEnv$SGPrepDataFileName , sep="")
projEnv$AlamodeInputTemplateFile <- paste(projEnv$TemplatesFolder,projEnv$AlamodeInputTemplateFileName, sep="")
projEnv$RcaInputTemplateFile <- paste(projEnv$TemplatesFolder,projEnv$RcaInputTemplateFileName, sep="")
projEnv$RcaInputFile <- paste(projEnv$MlsFolder,projEnv$RcaInputFileName, sep="")
## 


projEnv$LogFolder <- paste(projEnv$CodeFolder,"Log/",sep="")
# need to laod a second time - some bug
library(Rcpp)
...
source("1_Code/Module1/R/Alamode_URAR.R")
source("1_Code/Module1/R/RCA.R")

# Set up Version Folder System
projEnv$MlsVersionFolder  <- "C:/Order_1/B/A/PVN2023/KVE23_04_Pacifica_XXXXXXXXX/Earth/2_Data/Mls/VER20230504_213600_FINAL/"
projEnv$AlamodeInputFile <- paste(projEnv$MlsVersionFolder,projEnv$AlamodeInputFileName, sep="")
projEnv$RcaInputFile <- paste(projEnv$MlsVersionFolder,projEnv$RcaInputFileName, sep="")
proceed <- TRUE
...
projEnv$SGPrepDataFile <- paste(projEnv$MlsVersionFolder,"/SGPrepData.xlsx", sep="")
 

 if (!file.exists(projEnv$SGPrepDataFile)){
       flog.info( "Specified SGPrepData.xlsx File Does Not Exist." )  
	    flog.info( "Copy MlsData.xlsx to SGPrepData.xlsx and rename Sheet1 to 'X'." )  
